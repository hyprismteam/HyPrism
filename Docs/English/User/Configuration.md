...
- The Avalonia visual theme keeps its fixed white accent. Appearance places Personalization before a separate Background gallery. The gallery derives its column count from the available card width and distributes tile width evenly, so wider windows show more wallpapers per row without a fixed empty gutter. Its first three-way split thumbnail selects automatic background rotation. A selection is persisted and applied to the Dashboard immediately. Background music lives under General → Miscellaneous.

### Native Dashboard (Avalonia preview)

- The native home page is a focused launch screen rather than a statistics dashboard. The selected instance, its branch/version and readiness state sit directly over a masked full-surface background. The image, both dimming gradients, and the page content share one inner rounded surface aligned with the outer content shell, so no overlay can draw its own edge across a corner.
...
Settings use borderless controls throughout the native UI. Switches show state through a gray or green track without separate On/Off labels, and their knob moves between states with eased animation; drop-downs and text fields use smooth hover feedback without focus outlines. Language flags are derived from `country-flag-icons` 1.6.20 under the MIT license.
...
- News uses the same dark-gray surface as the other native pages. Every fetched post, including the newest one, uses the same flat list row without a section heading, redundant source badge, edge arrow, or hover/selection outline. Rows gain only a subtle animated hover surface and a persistent selected surface; repeated clicks on the selected row are ignored. Short titles allow a two-line summary, while wrapped titles reserve more room for the headline. Feed cards use the same 14 px horizontal and 18 px vertical navigation-surface inset and 2 px row spacing as Settings, while the slim scrollbar keeps its own reserved gutter. **Load More** at the end requests the next page without adding a hover background; only its text color changes smoothly.
