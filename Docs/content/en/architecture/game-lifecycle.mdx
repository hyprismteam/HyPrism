...
`GameInstallationWorkflow` rejects concurrent installation work for the same instance. It records `PendingVersion` before modifying an installation so an interrupted patch can be resumed. Fixed installed versions take a fast path; latest-version instances can request differential updates
