...
Desktop caps the Skia GPU resource cache at 256 MiB and uses sustained-low-latency garbage collection. Windows selects a high-performance rendering adapter with software fallback. These launcher-rendering choices are separate from the user's game GPU selection
