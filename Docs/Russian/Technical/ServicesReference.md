...
### UpdateService
- **Файл:** `Sources/HyPrism.Core/Core/App/UpdateService.cs`
- **Назначение:** Проверяет наличие нового релиза лаунчера и выполняет самообновление.
- **Источник обновлений:** `hyprismteam/HyPrism` через endpoint GitHub Releases `releases/latest`.
- **Политика релизов:** учитывается только последний опубликованный релиз. Каналы обновления лаунчера и выбор предрелизов не поддерживаются.
- **Пользовательский сценарий:** при запуске более новая версия отображается на Dashboard; установка выполняется внутри приложения с отображением прогресса, после чего лаунчер перезапускается.
