...
namespace HyPrism.Core.Tests.Core.Integration;
...
public sealed class NewsServiceTests
{
    ...
    public async Task GetNewsAsync_ParsesSemanticHytaleArticleCards()
    {
        ...
        var items = await service.GetNewsAsync(1, NewsSource.Hytale);
    ...
    public async Task GetNewsArticleAsync_DoesNotSerializeDifferentArticleDownloads()
    {
        ...
        var articles = await Task.WhenAll(first, second).WaitAsync(TimeSpan.FromSeconds(3));
    ...
    public async Task NewsAndArticlesSurviveServiceRestartInFileCache()
    {
        ...
        try
        {
            ...
            using (var client = new HttpClient(handler))
            {
                var service = new NewsService(client, new AppPathConfiguration(appDirectory));
                Assert.NotEmpty(await service.GetNewsAsync(12, NewsSource.Hytale));
            ...
            var restartedService = new NewsService(
                offlineClient,
                new AppPathConfiguration(appDirectory));

            Assert.NotEmpty(await restartedService.GetNewsAsync(12, NewsSource.Hytale));
